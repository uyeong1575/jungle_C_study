...
{
	/* add your code here */
	/* ll2를 하나씩 순회 하면서 해당 하는 위치에 삽입
	for(i=1부터 하면) ll1의 index = 2i 에 넣으면 된다.
	순회해서 넣은 후 i값에 따라서 index = 0에 있는 값을 삭제한다.*/

	ListNode *temp;
	temp = ll2->head;
	int index = 0; // 몇개 넣었는지 기록
	...
	for (int i = 1; i <= index; i++) // 넣은 만큼 ll2 0번 삭제 반복
	{
		removeNode(ll2, 0);
	}
}
