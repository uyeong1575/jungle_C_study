...
{
	/* add your code here */
	/* ptrHead는 head를 가리키는 포인터
	ll을 순회하면서 max값의 index를 저장한다.
	해당 index를 0에 넣고 index는 삭제*/

	ListNode *cur, *temp, *head;
	head = *ptrHead;
	int max_value = head->item;
	int max_index = 0;
	int index = 0;
	// max 노드 index 찾기
	cur = *ptrHead;
	while (cur != NULL)
	{
		if (max_value < cur->item)
		{
			max_value = cur->item; // max_value에 max값 업데이트
			max_index = index;	   // max_index에 max값의 index 업데이트
		}
		cur = cur->next;
		index++;
	}

	if (max_index == 0) // 0번이 max면 그냥 리턴
		return 0;

	// max 전 노드 찾기, temp에 저장
	int i = max_index - 1;
	temp = *ptrHead;
	while (i > 0)
	{
	...
	/*temp 다음 노드 삭제 = temp 노드를 다음 다음 노드에 연결
	후 cur 노드를 head로 만들기*/
	cur = temp->next;
	temp->next = cur->next;
	cur->next = head;
