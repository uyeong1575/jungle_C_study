...
{
	/* add your code here */
	/* size 확인해서 짝수면 (index = len/2)부터 back, 홀수면 (index = len/2+1)부터 back
	(size - index)개수만큼 반복해서 index의 노드를 backlist에 넣고 삭제
	마지막에 frontlist = ll*/
	...
	// 몇번째 index부터 backlist에 넣어야하는지 계산
	if (len % 2 == 0)
	{ // 짝수면
		index = len / 2;
	}
	...
	for (int i = 0; i < len - index; i++) // (ll 길이 - index)번 반복해서 ll의 index를 back으로 저장
	{
		insertNode(resultBackList, i, findNode(ll, index)->item);
		removeNode(ll, index);
	}
