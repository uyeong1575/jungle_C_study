...
void levelOrderTraversal(BSTNode *root)
{
	...
	Queue my_q = {NULL, NULL};
	BSTNode *cur;

	enqueue(&(my_q.head), &(my_q.tail), root);

	while (my_q.head != NULL)
	{

		cur = dequeue(&my_q.head, &my_q.tail);

		printf(" %d", cur->item);

		if (cur->left != NULL)
			enqueue(&(my_q.head), &(my_q.tail), cur->left);
		if (cur->right != NULL)
			enqueue(&(my_q.head), &(my_q.tail), cur->right);
	}
}
...
BSTNode *dequeue(QueueNode **headPtr, QueueNode **tailPtr) // 선입 선출, 즉 head를 지워야함
{
	BSTNode *node = (*headPtr)->data;
	QueueNode *tempPtr = *headPtr;
	*headPtr = (*headPtr)->nextPtr; // 다음을 해드로 옮김

	if (*headPtr == NULL) // 뒤로 갔더니 노드가 없어, 즉 지금 노드가 1개만 있는경우야
	{
		*tailPtr = NULL; // tail도 null로 만들어야함.
	}

	free(tempPtr); // 지우려는 노드(해드)를 free

	return node; // 지운 노드 주소 리턴
}
